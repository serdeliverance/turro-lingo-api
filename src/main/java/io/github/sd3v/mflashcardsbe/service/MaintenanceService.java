...
public class MaintenanceService {
    ...
    public void addIdToFlashcards() {
        deckService.getAll()
                ...
                .map(deck -> {
                    var flashcards = deck.flashcards().stream().map(f ->
                        new Flashcard(UUID.randomUUID().toString(),f.front(), f.back())
                    ).toList();
                    return new Deck(deck.id(), deck.name(), deck.slug(), deck.description(), deck.type(), deck.language(), flashcards, deck.tags());
