...
public class DeckService {
    ...
    public Deck create(CreateDeck createDeck) {
        ...
        var flashcardEntityList = createDeck.flashcards().stream().map(flashcard -> flashcardRepository.save(flashcard, deckEntity.id())).toList();
    ...
    private Deck buildDeckWithFlashcards(DeckEntity entity) {
        ...
        var flashcardList = flashcardRepository.findAllByDeckId(entity.id()).stream().map(flashcard -> new Flashcard(flashcard.id(), flashcard.front(), flashcard.back()) ).toList();
        var tags = Arrays.stream(entity.tags().split(",")).map(String::trim).toList();
        return new Deck(entity.id(), entity.name(), entity.slug(), entity.description(), entity.type(), entity.language(), flashcardList, tags);
    }
